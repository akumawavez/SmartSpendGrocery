...
with tab2:
    ...
    st.markdown("Search for products on Albert Heijn and compare prices with and without AH membership.")
    ...
    with col_slider:
        max_results = st.slider("Max results", min_value=10, max_value=50, value=20, key="max_results")
            ...
            with st.spinner(f"Searching for '{search_query}' on Albert Heijn..."):
                ...
                if products:
                    ...
                    if len(products_by_category) > 1:
                        category_tabs = st.tabs(list(products_by_category.keys()))
                    else:
                    ...
                    for idx, (category, category_products) in enumerate(products_by_category.items()):
                        with category_tabs[idx]:
                            st.subheader(f"📦 {category} ({len(category_products)} products)")
                            ...
                            for product in category_products:
                                ...
                                with row_cols[2]:
                                    ...
                                    price_without = product.get('price_without_membership', 0)
                                    st.markdown(f"**€{price_without:.2f}**")
                                
                                with row_cols[3]:
                                    # Price with membership
                                    price_with = product.get('price_with_membership', price_without)
                                    savings = price_without - price_with if price_with < price_without else 0
                                    
                                    if price_with < price_without:
                                        st.markdown(f"**€{price_with:.2f}**")
                                        st.caption(f"💵 Save: €{savings:.2f}")
                                    else:
                                        st.markdown(f"**€{price_with:.2f}**")
                                
                                with row_cols[4]:
                                    # Discount offers
                                    discount_offer = product.get('discount_offer', '')
                            ...
                            if len(category_products) > 0:
                                ...
                                with cat_col1:
                                    avg_no_member = sum([p.get('price_without_membership', 0) for p in category_products]) / len(category_products)
                                    st.metric("Avg (No Membership)", f"€{avg_no_member:.2f}")
                                
                                with cat_col2:
                                    avg_member = sum([p.get('price_with_membership', p.get('price_without_membership', 0)) for p in category_products]) / len(category_products)
                                    st.metric("Avg (With Membership)", f"€{avg_member:.2f}")
                                
                                with cat_col3:
                                    total_savings_cat = sum([max(0, p.get('price_without_membership', 0) - p.get('price_with_membership', p.get('price_without_membership', 0))) for p in category_products])
                                    st.metric("Total Savings", f"€{total_savings_cat:.2f}")
                    ...
                    if len(products_by_category) > 1:
                        ...
                        with summary_col2:
                            avg_price_no_member = sum([p.get('price_without_membership', 0) for p in products]) / len(products)
                            st.metric("Avg Price (No Membership)", f"€{avg_price_no_member:.2f}")
                        
                        with summary_col3:
                            avg_price_member = sum([p.get('price_with_membership', p.get('price_without_membership', 0)) for p in products]) / len(products)
                            st.metric("Avg Price (With Membership)", f"€{avg_price_member:.2f}")
                        
                        with summary_col4:
                            total_savings = sum([max(0, p.get('price_without_membership', 0) - p.get('price_with_membership', p.get('price_without_membership', 0))) for p in products])
                            st.metric("Total Potential Savings", f"€{total_savings:.2f}")
                else:
                    st.warning("No products found. Try a different search query or check your internet connection.")
                    st.info("💡 Tip: Try searching for common products like 'bananas', 'milk', 'bread', or 'chicken'")
        else:
            st.info("👆 Enter a product name above to search for prices on Albert Heijn.")
